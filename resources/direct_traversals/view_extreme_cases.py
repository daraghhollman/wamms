...
def main():
    ...
    # We want to find the extreme events
    extreme_cutoff = 4  # Hours
    extreme_traversals = traversals.loc[
        traversals["Length (seconds)"] > extreme_cutoff * 3600
    ]

    print(len(extreme_traversals))

    print(extreme_traversals["Length (seconds)"] / 3600)

    for i, traversal in extreme_traversals.iterrows():
...
def create_figure(traversal):
    ...
    if traversal["Start Time"] < dt.datetime(2012, 4, 1):
        ...
        buffer = dt.timedelta(hours=4)

    start = traversal["Start Time"] - buffer
    end = traversal["End Time"] + buffer
    ...
    data_within_traversal = data.loc[
        data["date"].between(traversal["Start Time"], traversal["End Time"])
    ]
    dwt = data_within_traversal
    ...
    xc_axis.plot(
        dwt["X MSM' (radii)"],
        np.sqrt(dwt["Y MSM' (radii)"] ** 2 + dwt["Z MSM' (radii)"] ** 2),
        color=plotting.wong_colours["pink"],
        lw=3,
    )
